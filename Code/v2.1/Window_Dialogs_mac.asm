...
;
DeactivateWnd   jsr PaintCurWndDeac
                jmp WindowToScreen
...
; and title in FDFE
ShowYesNoDlg    jsr SaveFBFD_Deact
                ldx #<Wnd_Dlg_YesNo
                ldy #>Wnd_Dlg_YesNo
                jsr CreateWindow
                ldx $fd
                ...
                ldy #>Ctrl_YN_NoBtn
                jsr AddControl
                lda #(BIT_CTRL_DBLFRAME_RGT + BIT_CTRL_DBLFRAME_BTM + BIT_CTRL_DBLFRAME_LFT)
                jsr AddBits_Adjust
                ;jsr AdjustCtrlPos
                ;lda #(BIT_CTRL_DBLFRAME_RGT + BIT_CTRL_DBLFRAME_BTM + BIT_CTRL_DBLFRAME_LFT)
                ;ora ControlBits
...
AddBits_Adjust  ora ControlBits
                sta ControlBits
                jsr AdjustCtrlPos
                jmp UpdateControl

; Creates message with multi-line string in FBFC
CreateMsgDlg    jsr SaveFBFD_Deact
                ;
...
; Shows error message with multi-line string in Addr(X,Y)
ShowErrorMsg    stx $fb
                sty $fc
                jsr CreateMsgDlg
                ldx #<Str_Dlg_Error
...
CreateDevNoDlg  jsr DeactivateWnd
                ...
                jsr CreateWindowEx
                jsr SelectControl1
                lda #(BIT_CTRL_DBLFRAME_TOP + BIT_CTRL_DBLFRAME_BTM)
                jsr SetControlBits
                jsr SelectControl2
                lda #(BIT_CTRL_DBLFRAME_TOP + BIT_CTRL_DBLFRAME_BTM)
                ;jmp SetControlBits

; Sets control bits in A and updates control
SetControlBits  ora ControlBits
                sta ControlBits
                jmp UpdateControl
...
ShowClockDialog jsr DeactivateWnd
                ...
                ;lda #(BIT_CTRL_DBLFRAME_RGT + BIT_CTRL_DBLFRAME_LFT + BIT_CTRL_DBLFRAME_BTM + BIT_CTRL_DBLFRAME_TOP)
                ;;ora ControlBits
                ;sta ControlBits
                ;ldy #CL_WHITE
                ;jsr SetCtrlColor
...
-               txa
                ...
                ;
                ldy #2
                lda #(BIT_CTRL_DBLFRAME_LFT + BIT_CTRL_DBLFRAME_RGT + BIT_CTRL_DBLFRAME_TOP + BIT_CTRL_DBLFRAME_BTM)
                jsr SelCtrl_AddBits
                ;
                jmp ShowTheDialog
