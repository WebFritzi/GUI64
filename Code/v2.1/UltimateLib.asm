...
DATA_ADDR = SMC_UltimatDat+1
max_one_block   !byte 0
U_ReadDataPack  ldx #0
-               lda STATUS_REG
                and #SR_BIT_DATA_AV
                bne +
                lda #0
                sta max_one_block
                beq U_Accept; jmp U_Accept
+               ; Data available
                lda RESP_DATA_REG
SMC_UltimatDat  sta $FFFF,x
                inx
                bne -
                inc DATA_ADDR+1
                lda max_one_block
                beq -
                ; Default: max_one_block = 0
                lda #0
                sta max_one_block
                beq U_Abort; jmp U_Abort
...
U_SendCommandAndGetStatus
                jsr U_SendCommand
                jsr U_ReadStatusMsg
                jmp U_Accept

U_FREEMEM_TO_DATA_ADDR
                lda #<FREEMEM
                sta DATA_ADDR
                lda #>FREEMEM
                sta DATA_ADDR+1
...
U_GetTime       lda #DOS_CMD_GET_TIME
                jsr U_PrepareCmd
                jsr U_SendCommand
...
!zone Path
; Retrieves the current path to &U_Path
; Output:
; * Path address in FBFC
; * Path length: LO=X, HI=NOT(Y)
; * Zero flag: -beq: Ok
;              -bne: overflow (more than 256 chars read)
U_GetPath       ldx U_CurTarget
                dex
                lda U_PathLo,x
                sta $fb
                sta DATA_ADDR
...
; * filename length in U_len
U_MountImage    lda #DOS_CMD_MOUNT_DISK
                jsr U_PrepareCmd
                lda U_MountDevNo
                sta FREEMEM+2
                jsr U_SendCommandWithULen
                and #%00001111
                rts

; Helper
U_SendCommandWithULen
                lda U_len
                clc
                adc #3
                sta cmd_len
                jsr U_SendCommandAndGetStatus
                lda FREEMEM
                rts
...
; Output in zero flag (beq: "00,OK", bne: error, status msg in FREEMEM)
; X = Length of status msg
U_OpenFile      lda #DOS_CMD_OPEN_FILE
                jsr U_PrepareCmd
SMC_U_Open      lda #$FF
                sta FREEMEM+2
                jsr U_SendCommandWithULen
...
;   1 byte: attribute
;   x bytes: filename
;U_GetFileInfo   lda #DOS_CMD_FILE_INFO
;                jsr U_PrepareCmd
;                sta FREEMEM+1
;                lda #2
;                sta cmd_len
...
U_ReadSmallFile lda #DOS_CMD_READ_DATA
                jsr U_PrepareCmd
                lda #$00
                sta FREEMEM+2
                lda #$02
...
; Closes opened file
U_CloseFile     lda #DOS_CMD_CLOSE_FILE
                jsr U_PrepareCmd
                jsr U_SendCommand
                jmp U_Accept
...
U_ChangeDir     lda #DOS_CMD_CHANGE_DIR
                jsr U_PrepareCmd
                ldx U_len
                inx
                inx
...
; * 1: Empty directory
; * 6: Can't open error
U_OpenDir       lda #DOS_CMD_OPEN_DIR
                jsr U_PrepareCmd
                jsr U_SendCommandAndGetStatus
                lda FREEMEM+1
                and #%00000111
...
; * 0: Ok
; * 3: No such directory
U_GoUpDir       lda #DOS_CMD_CHANGE_DIR
                jsr U_PrepareCmd
                lda #'.'
                sta FREEMEM+2
                sta FREEMEM+3
...
!zone Conversions
AsciiToPetscii  cmp #$40
                bcc ascii_done        ; $20-$3f remain unchanged
                beq ascii_at          ; $40 -> $00
                cmp #$5b
                bcc ascii_letter      ; $41-$5a -> $61-$7a
                cmp #$60
                bcc ascii_5x          ; $5b-$5f special chars
                beq ascii_grave       ; $60 -> $27
                cmp #$7b
                bcc ascii_letter      ; $61-$7a -> $41-$5a
                ; $7b-$7f
                cmp #$7c
                beq ascii_pipe        ; $7c -> $7d
                lda #$3f              ; $7b,$7d,$7e,$7f -> "?"
ascii_done      rts
ascii_letter    eor #$20              ; swap upper and lower case
                rts
ascii_at        lda #$00
                rts
ascii_grave     lda #$27              ; ` -> '
                rts
ascii_pipe      lda #$7d              ; | -> PETSCII $7d
                rts
; Special chars $5b-$5f:
; $5b -> $5b
; $5c -> $2f
...
ascii_5x        cmp #$5c
                ...
                beq ascii_caret
                cmp #$5f
                bne ascii_done        ; $5b and $5d remain
                lda #$a4              ; _
                rts
ascii_backslash lda #$2f              ; \ -> /
                rts
ascii_caret     lda #$1e              ; ^
                rts
; Use as follows:
