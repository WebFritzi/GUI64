...
; * CurDeviceNo filled
; * max_entries filled
LoadDir         jsr DefHourglassChars
                lda #1
                sta HourglassCount
                lda #5
                sta MHourglassCount
                lda #0
                sta ExHourglassCount
                jsr SetHourglassPos
                jsr PaintHourglass
                ;
                jsr error_codeTo0
...
++              ; Not a disk drive
                lda IsDiskImage,x
                bne +
                ; Folder (or virtual device in VICE)
                ; Works only for d64 images as virtual device in VICE
                jsr Prepare1541
                jmp rename_disk
-               rts
+               ; Disk image
